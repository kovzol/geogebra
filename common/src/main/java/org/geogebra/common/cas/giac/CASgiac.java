...
public abstract class CASgiac implements CASGenericInterface {
	...
	public static enum CustomFunctions {
		...
		JACOBI_DET("jacobiDet", ggbGiac("jacobiDet(polys,excludevars)->"
				...
				+ "  sx:=size(polys);"
				+ "  if (sx!=sy) {"
				+ "    print(\"Internal error: Inequal number of polys/vars\",sy,sx);"
				+ "    return(0);"
				+ "  }"
				+ "  J:=matrix(sx,sy, (j,k)=>diff(polys[j],vars[k]));"
				+ "  print(J);"
				+ "  dm:=det_minor(J);"
		...
		public static void setDependencies() {
			...
			setDependency(JACOBI_PREPARE, IS_LINEAR);
			setDependency(IS_LINEAR, IS_LINEAR_SUM);
	...
	public String createEliminateFactorizedScript(String polys,
		...
		return script.append("[" + "[ff:=\"\"],[aa:=").append(eliminateCommand)
				.append("],")
				.append("[bb:=size(aa)],[for ii from 0 to bb-1 do ff+=(\"\\n[\"+(ii+1)+\"]:\\n [1]:\\n ")
				.append(" _[1]=1\");ee:=aa[ii]/gcd(coeffs(aa[ii],lvar(aa[ii])));cc:=factors(ee);dd:=size(cc);")
				.append("for jj from 0 to dd-1 by 2 do ff+=(\"\\n  _[\"+(jj/2+2)+\"]=\"+cc[jj]); od; ff+=(\"\\n [2]: ")
