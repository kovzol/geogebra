...
public class ConstructionProtocolView implements ConstructionStepper {
	...
	/**
	 * Returns a latex representation of the construction protocol.
	 * 
	 * @param loc
	 *            localization
	 * @param kernel
	 ...
	 * @return latex
	 */
	public static String getLatex(Localization loc,
			Kernel kernel, ArrayList<Columns> columns, boolean useColors) {

		StringBuilder sb = new StringBuilder();

		sb.append("% This LaTeX file was generated by GeoGebra.\n");
		sb.append("% Before processing it, you will need to have the mode icon files in this folder.\n");
		sb.append("% They are available in the source code at https://github.com/geogebra/geogebra\n");
		sb.append("% in folder common/src/nonfree/resources/org/geogebra/common/icons_toolbar/p64/.\n");
		sb.append("\n");
		sb.append("% You may need to run LaTeX twice to get the proper size for the table.\n");
		sb.append("\n");
		sb.append("\\documentclass{article}\n");
		sb.append("\\usepackage[margin=2cm]{geometry} % please fine tune manually if needed\n");
		sb.append("\\usepackage{longtable}\n");
		sb.append("\\usepackage{color}\n");
		sb.append("\\usepackage{graphicx}\n");
		sb.append("\\usepackage{booktabs}\n");
		sb.append("\\usepackage{amssymb} % for \\checkmark, see https://tex.stackexchange.com/questions/132783/how-to-write-checkmark-in-latex\n");
		sb.append("\\title{");
		sb.append(StringUtil.toHTMLString(GeoGebraConstants.APPLICATION_NAME));
		sb.append(" -- ");
		sb.append(loc.getMenu("ConstructionProtocol"));
		...
		for (int nRow = 0; nRow < endRow; nRow++) {
			...
			GColor color = useColors ? geo.getAlgebraColor()
					: GColor.BLACK;
			if (!GColor.BLACK.equals(color)) {
				sb.append("\\definecolor{" + colorname + "}{RGB}{");
