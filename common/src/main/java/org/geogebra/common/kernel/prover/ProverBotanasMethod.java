	...
	public static class AlgebraicStatement {
		...
		public void addGeoPolys(GeoElement geo, PPolynomial[] ps) {
			...
			for (PPolynomial p : ps) {
				if (addPolynomial(p)) {
					if (ProverSettings.get().captionAlgebra || geoProver.getCaptionalgebra()) {
			...
			while (it.hasNext()) {
				...
				if (geo.equals(numerical)) {
					...
					try {
						...
						if (proverSettings.captionAlgebra || geoProver.getCaptionalgebra()) {
							geo.setCaption(null);
						}
						String command = geo
								...
								try { // Show detailed descriptions of variables first...
									...
									if (geoProver.getShowproof() && descriptions != null) {
										...
										if (descriptiveLines > 0 ) {
											geoProver.addProofLine(loc.getPlain("ObjectAIntroduces",
													geo.getLabelSimple()));
											for (int i = 0; i < descriptions.length; i++) {
												if (descriptions[i] != null) {
		...
		private void setThesis() {
			...
			if (geoProver.getConstruction() != null) {
				...
				if (geoProver.getShowproof()) {
					geoProver.addProofLine(loc.getMenuDefault("ThesisEqsNonDenied",
				...
				if (geoProver.getShowproof()) {
					geoProver.addProofLine(loc.getMenuDefault("ThesisEqDenied",
	...
	public static void syzygy(AlgebraicStatement as, PPolynomial ndgproduct, TreeMap<PVariable, BigInteger> substitutions,
		...
		if (singularWS != null && singularWS.isAvailable()) {
			...
			if (syzygyResult != null && !syzygyResult.equals("")) {
				...
				prover.addProofLine(loc.getMenuDefault("NowConsider", "Now we consider the following equation:"));
			...
			*/
			...
			syzygyProgram += "[v:=gbasis_vars(I1)],";
